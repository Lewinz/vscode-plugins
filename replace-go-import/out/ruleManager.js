...
class RuleManager {
    ...
    validateRules(rawRules) {
        ...
        for (let i = 0; i < rawRules.length; i++) {
            ...
            if (Array.isArray(rule.exemptPaths)) {
                exemptPaths = rule.exemptPaths.filter((path) => typeof path === 'string');
            }
            validRules.push({
                name: rule.name.trim(),
                from: rule.from.trim(),
    ...
    applyRule(line, rule, lineIndex) {
        try {
            ...
            if (this.config?.showNotifications) {
                vscode.window.showErrorMessage(`规则 "${rule.name}" 执行失败: ${errorMsg}`);
            }
            return null;
        }
    }
     ...
     * 应用更改到文档
     */
    async applyChanges(document, result) {
        ...
        if (success && this.config?.showNotifications) {
            vscode.window.showInformationMessage(`已应用 ${result.replacements.length} 个替换规则`, '查看详情').then((selection) => {
                if (selection === '查看详情') {
     ...
     * 显示替换详情
     */
    showReplacementDetails(replacements) {
        const panel = vscode.window.createWebviewPanel('replacementDetails', '替换详情', vscode.ViewColumn.Two, {});
     ...
     * 生成详情HTML
     */
    generateDetailsHtml(replacements) {
