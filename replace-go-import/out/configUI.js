...
class ConfigUI {
    ...
    openSettings() {
        ...
        this.panel = vscode.window.createWebviewPanel('replaceGoImportSettings', 'Replace Go Import 设置', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async testRules() {
        ...
        const selectedCase = await vscode.window.showQuickPick(testCases.map(case_ => ({
            label: case_.name,
    ...
    async saveRules(rules) {
        try {
            ...
            if (validatedRules.length !== rules.length) {
                ...
                vscode.window.showWarningMessage(`保存完成，但有 ${invalidCount} 个规则因格式错误被忽略`);
            }
            ...
            vscode.window.showInformationMessage(`已保存 ${validatedRules.length} 个有效规则`);
        }
        catch (error) {
     ...
     * 测试单个规则
     */
    async testRule(rule, testText) {
        const result = this.ruleManager.testRule(rule, testText);
        if (result.success) {
            vscode.window.showInformationMessage(`规则 "${rule.name}" 测试成功`, '查看结果').then((selection) => {
                if (selection === '查看结果') {
            ...
            vscode.window.showErrorMessage(`规则 "${rule.name}" 测试失败: ${result.error}`);
        }
    }
    /**
     * 显示输入框
     */
    async showInputBox(message) {
     ...
     * 使用规则测试所有用例
     */
    async testAllCasesWithRule(message) {
     ...
     * 获取测试用例
     */
    getTestCases(fromPath = "github.com/company/repo") {
        return [
            {
                name: "单行import",
...
import (
     ...
     * 显示测试结果
     */
    showTestResults(testText, results) {
        const panel = vscode.window.createWebviewPanel('testResults', '规则测试结果', vscode.ViewColumn.Two, {});
     ...
     * 显示所有测试结果
     */
    showAllTestResults(rule, results) {
        const panel = vscode.window.createWebviewPanel('allTestResults', `规则测试结果: ${rule.name}`, vscode.ViewColumn.Two, {});
     ...
     * 显示单个测试结果
     */
    showSingleTestResult(rule, testText, result) {
        const panel = vscode.window.createWebviewPanel('singleTestResult', `规则测试: ${rule.name}`, vscode.ViewColumn.Two, {});
        panel.webview.html = `
            <!DOCTYPE html>
            <html>
            <head>
     ...
     * 生成规则HTML
     */
    generateRuleHtml(rule, index) {
        const exemptPathsHtml = rule.exemptPaths.map(path => `<span class="exempt-path">${this.escapeHtml(path)}</span>`).join('');
        return `
            <div class="rule-item" id="rule-${index}">
                <div class="rule-header">
